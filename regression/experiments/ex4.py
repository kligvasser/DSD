...
#sys.path.append(os.path.abspath('../../../utils/niqe/'))
#import niqe

import matlab.engine

def get_arguments():
    parser = argparse.ArgumentParser(description='PieAPP-noref-NIQE')
...
if __name__ == "__main__":
    ...
    for csv_file in csv_list:
        ...
        for j, data in enumerate(loader):
            ...
            # image_a = niqe.read_image(a_path)
            # score_a = niqe.niqe(image_a)
            image_a = eng.imread(a_path)
            score_a = float(eng.niqe(image_a))


            # image_b = niqe.read_image(b_path)
            # score_b = niqe.niqe(image_b)
            image_b = eng.imread(b_path)
            score_b = float(eng.niqe(image_b))
