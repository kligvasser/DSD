...
from models.modules.losses import PerceptualLoss, DSDLoss
...
class Trainer():
    ...
    def _init_model(self):
        ...
        # check vgg discriminator
        if self.args.dis_model.find('vgg') != -1:
            self.vgg_model = True
        ...
        if self.print_model:
            ...
            if self.vgg_model:
                if self.parallel:
                    logging.info('Number of parameters in discriminator: {}\n'.format(sum([l.nelement() for l in self.d_model.module.classifier.parameters()])))
                else:
                    logging.info('Number of parameters in discriminator: {}\n'.format(sum([l.nelement() for l in self.d_model.classifier.parameters()])))
            else:
    ...
    def _init_optim(self):
        ...
        if self.vgg_model:
            if self.parallel:
                self.d_optimizer = torch.optim.Adam(self.d_model.module.classifier.parameters(), lr=self.args.lr, betas=self.args.dis_betas)
            else:
                self.d_optimizer = torch.optim.Adam(self.d_model.classifier.parameters(), lr=self.args.lr, betas=self.args.dis_betas)
        else:
        ...
        if self.args.dsd_weight:
            self.dsd = DSDLoss(features_to_compute=self.args.features, scales=self.args.recurrent_scales).to(self.args.device)
            
    def _init(self):
        # init parameters
        self.step = 0
        self.losses = {'D': [], 'D_r': [], 'D_gp': [], 'D_f': [], 'G': [], 'G_recon': [], 'G_dsd': [], 'G_perc': [], 'G_adv': [], 'psnr': []}
    ...
    def _save_model(self, epoch):
        # save models
        torch.save(self.g_model.state_dict(), os.path.join(self.args.save_path, '{}_e{}.pt'.format(self.args.gen_model, epoch + 1)))
        torch.save(self.d_model.state_dict(), os.path.join(self.args.save_path, '{}_e{}.pt'.format(self.args.dis_model, epoch + 1)))
    ...
    def _generator_iteration(self, inputs, targets):
        ...
        # dsd loss
        if self.args.dsd_weight:
            loss_dsd = self.dsd(generated_data, targets)
            loss += loss_dsd * self.args.dsd_weight
            self.losses['G_dsd'].append(loss_dsd.data.item())
    ...
    def _train_iteration(self, data):
        ...
        if self.step % self.args.print_every == 0:
            ...
            if self.step > self.args.num_critic:
                ...
                if self.args.dsd_weight:
                    line2print += ', G_dsd: {:.8f}'.format(self.losses['G_dsd'][-1])
                if self.args.adversarial_weight:
