...
class StyleLoss(nn.Module):
    def __init__(self, features_to_compute=['relu1_2', 'relu2_1', 'relu3_1'], criterion=torch.nn.L1Loss(), shave_edge=None):
...
class DSDLoss(nn.Module):
    def __init__(self, features_to_compute=['relu1_2', 'relu2_1', 'relu3_1'], scales=[0.5], criterion=torch.nn.L1Loss(), shave_edge=None):
...
class MultiScaleStyleLoss(nn.Module):
    def __init__(self, features_to_compute=['relu1_2', 'relu2_1', 'relu3_1'], scales=[1.0, 0.5], criterion=torch.nn.L1Loss(), shave_edge=None):
        super(MultiScaleStyleLoss, self).__init__()
        self.scales = scales
        self.criterion = criterion
        self.features_extractor = MultiVGGFeaturesExtractor(target_features=features_to_compute, shave_edge=shave_edge).eval()
